...
import type { VerifiedCodingMutation, CodingMutationLearningContext } from './verified-coding-mutation.js';
...
export class AgentCodingSession {
  ...
  async run(
    ...
    taskId = 'coding-session',
    learningContext?: CodingMutationLearningContext
    ...
    for (let turn = 0; turn < this.limits.maxTurns; turn++) {
      ...
      for (const call of response.toolCalls) {
        ...
        const result = call.name === 'repo.write'
          ? await this.mutation.execute((call.arguments as { path: string }).path, (call.arguments as { content: string }).content, undefined, learningContext)
